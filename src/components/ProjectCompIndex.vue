    ...
    >
      ...
      <div class="hidden xl:block">
        ...
        >
          ...
          <div class="flex border border-white p-2 px-6 pb-1 rounded-[10rem] w-8/12 text-white space-x-5 hover:bg-colored transition-colors duration-500 ">
            <p class="text-sm font-unbounded font-light self-start">
              {{ subtitle }}
            </p>
            <Arrow class="w-4 -rotate-90"/>
          </div>
        </div>
      </div>
    </div>

    <h2 class="text-xl font-lactos md:text-2xl mt-2 xl:hidden">{{ title }}</h2>
    <p class="text-xs font-unbounded font-light text-coloblue hover:text-colored transition-colors duration-500 md:text-sm cursor-pointer xl:hidden">
      {{ subtitle }}
    </p>
  </router-link>
</template>
